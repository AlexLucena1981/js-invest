...
let strategiesDB = [];
let activeBrokers = {}; 
let availableCoins = ['btcusdt', 'ethusdt', 'solusdt', 'bnbusdt']; // 🌐 Fallback de Moedas

// ============================================================================
// 3. CARREGAMENTO DE DADOS (SCRIPTS E MOEDAS)
// ============================================================================
...
// 🌐 Busca Dinâmica de Moedas da Binance
                ...
                if (isCandleClosed) { 
                    ...
                    if (activeSignals.length === 0) {
                        ...
                        if (newSignalType) {
                            ...
                            activeSignals.push(newSig); signalHistory.unshift(newSig); if (signalHistory.length > 20) signalHistory.pop();
...
io.on('connection', (socket) => {
    ...
    socket.emit('available_coins', availableCoins); // 🌐 Envia a lista de moedas
    ...
    socket.on('hybrid_login', async ({ brokerUser, brokerPass }) => {
        try {
            ...
            if (!isSubscribed) {
                socket.emit('hybrid_login_result', { success: false, reason: 'subscription', msg: 'Assinatura Inativa. Adquira seu acesso ao JS Invest para continuar.' }); return;
            }
    ...
    socket.on('manual_trade', async (data) => {
        ...
        if (result.success) {
            ...
            const manualSig = { 
                ...
                time: new Date().toLocaleTimeString('pt-BR', { timeZone: 'America/Sao_Paulo' }), step: 0, status: '⚡ Sniper (Aguardando...)',
                entryPrice: currentGlobalPrice, isManual: true 
            };
...
loadAvailableCoins(); // 🌐 Carrega as moedas dinamicamente
