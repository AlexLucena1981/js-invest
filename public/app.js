...
function saveRiskConfig() {
    const config = { accountType: document.getElementById('riskAccount').value, baseAmount: document.getElementById('riskAmount').value, maxGale: document.getElementById('riskGale').value, stopWin: document.getElementById('riskWin').value, stopLoss: document.getElementById('riskLoss').value };
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedConfig) {
        document.getElementById('riskAccount').value = savedConfig.accountType || 'demo'; document.getElementById('riskAmount').value = savedConfig.baseAmount || 5;
        document.getElementById('riskGale').value = savedConfig.maxGale || 2; document.getElementById('riskWin').value = savedConfig.stopWin || 50; document.getElementById('riskLoss').value = savedConfig.stopLoss || 50;
    }

    // TENTA O LOGIN SILENCIOSO SE TIVER O PASSE VIP
    ...
    if (savedBrokerToken) {
        ...
        socket.emit('auto_reconnect', { token: savedBrokerToken, role: savedRole });
    }
});

['riskAccount', 'riskAmount', 'riskGale', 'riskWin', 'riskLoss'].forEach(id => { document.getElementById(id).addEventListener('change', saveRiskConfig); });
...
// A ROTA DE RECONEXÃO APÓS O F5 (Abre as portas direto)
socket.on('auto_reconnect_result', (res) => {
    if(res.success) {
        ...
        // Se a sessão expirou, rasga o Passe VIP
        localStorage.removeItem('jsInvestBrokerToken'); localStorage.removeItem('jsInvestUserRole');
        document.getElementById('btnLogin').innerText = "Acessar Sistema";
    }
});

document.getElementById('btnLogout').addEventListener('click', () => { 
    // 🧹 RASGA O PASSE VIP AO SAIR
    localStorage.removeItem('jsInvestBrokerToken'); localStorage.removeItem('jsInvestUserRole');
...
document.getElementById('btnToggleBot').addEventListener('click', () => {
    ...
    const config = { active: isBotActive, accountType: document.getElementById('riskAccount').value, baseAmount: parseFloat(document.getElementById('riskAmount').value), maxGale: parseInt(document.getElementById('riskGale').value), stopWin: parseFloat(document.getElementById('riskWin').value), stopLoss: parseFloat(document.getElementById('riskLoss').value) };
...
socket.on('auto_trade_status', (res) => {
    ...
    if (res.profit !== undefined) { const pVal = document.getElementById('profitVal'); pVal.innerText = `R$ ${res.profit.toFixed(2)}`; pVal.style.color = res.profit >= 0 ? "#3fb950" : "#f85149"; }
});
...
socket.on('win_balance_update', (data) => {
    const el = document.getElementById(data.isDemo ? 'valDemo' : 'valReal'); let currentVal = parseFloat(el.innerText.replace('R$ ', '').replace(/\./g, '').replace(',', '.'));
    if (!isNaN(currentVal)) { el.innerText = `R$ ${(currentVal + data.prize).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`; el.style.color = '#3fb950'; el.style.textShadow = '0 0 15px rgba(63, 185, 80, 0.8)'; setTimeout(() => { el.style.color = data.isDemo ? '#d29922' : '#3fb950'; el.style.textShadow = 'none'; }, 2000); }
});
...
document.getElementById('btnManualCall').addEventListener('click', () => { 
    if(confirm("Confirma COMPRA manual?")) { saveRiskConfig(); const config = { accountType: document.getElementById('riskAccount').value, baseAmount: parseFloat(document.getElementById('riskAmount').value), maxGale: parseInt(document.getElementById('riskGale').value) }; const telaMoeda = document.getElementById('coinSelector').value; const telaTempo = document.getElementById('timeframeSelector').value; socket.emit('manual_trade', { direction: 'CALL', config: config, symbol: telaMoeda, timeframe: telaTempo }); } 
});

document.getElementById('btnManualPut').addEventListener('click', () => { 
    if(confirm("Confirma VENDA manual?")) { saveRiskConfig(); const config = { accountType: document.getElementById('riskAccount').value, baseAmount: parseFloat(document.getElementById('riskAmount').value), maxGale: parseInt(document.getElementById('riskGale').value) }; const telaMoeda = document.getElementById('coinSelector').value; const telaTempo = document.getElementById('timeframeSelector').value; socket.emit('manual_trade', { direction: 'PUT', config: config, symbol: telaMoeda, timeframe: telaTempo }); } 
});
...
socket.on('history_dump', (historyArr) => {
    ...
    historyArr.forEach(sig => {
        ...
        let colorClass = 'text-warning'; if (sig.status.includes('WIN')) colorClass = 'text-green'; else if (sig.status.includes('LOSS')) colorClass = 'text-red'; else if (sig.status.includes('Sniper') || sig.status.includes('Manual')) colorClass = 'text-warning';
...
socket.on('new_signal_history', (sig) => {
    ...
    let colorClass = 'text-warning'; if (sig.status.includes('Sniper') || sig.status.includes('Manual')) colorClass = 'text-warning';
...
socket.on('signal_result', (sig) => { const resTd = document.getElementById(`res-${sig.id}`); if (resTd) { resTd.innerText = sig.status; if (sig.status.includes('WIN')) resTd.className = 'text-green'; else if (sig.status.includes('LOSS')) resTd.className = 'text-red'; else resTd.className = 'text-warning'; } });
...
socket.on('script_injection_result', (res) => { if(document.getElementById('btnSaveScript')) document.getElementById('btnSaveScript').innerText = 'Salvar & Injetar'; if (res.success) { alert("✅ " + res.msg); scriptModal.style.display = 'none'; setTimeout(() => { document.getElementById('strategySelector').value = res.id; socket.emit('change_strategy', res.id); }, 500); } else { alert("❌ Erro:\n" + res.msg); } });
...
socket.on('admin_users_list', (res) => { const tbody = document.getElementById('usersListBody'); tbody.innerHTML = ''; if (res.success) { if (res.users.length === 0) tbody.innerHTML = '<tr><td colspan="2" style="text-align:center;">Vazio.</td></tr>'; else { res.users.forEach(u => { const tr = document.createElement('tr'); tr.innerHTML = `<td style="padding: 10px; border-bottom: 1px solid #21262d;">${u.email}</td><td style="padding: 10px; border-bottom: 1px solid #21262d; color: ${u.role.includes('admin') ? '#58a6ff' : '#8b949e'};">${u.role.toUpperCase()}</td>`; tbody.appendChild(tr); }); } } });
...
socket.on('user_creation_result', (res) => { alert(res.msg); if(document.getElementById('btnCreateUser')) document.getElementById('btnCreateUser').innerText = 'Cadastrar'; if(res.success) { document.getElementById('newUserEmail').value = ''; document.getElementById('newUserPassword').value = ''; auth.currentUser.getIdToken().then(token => socket.emit('admin_get_users', token)); } });
